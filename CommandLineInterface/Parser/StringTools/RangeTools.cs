...
namespace CommandLineInterface
{
    public static partial class Parser
    {
        ...
        public static partial class StringTools
        {
            ...
            public static bool InRange(int num, (int, int) range)
            {
                return range.Item1 <= num && range.Item2 >= num;
            }

            public static void PrintRanges(string text, List<(int, int)> ranges)
            {
                foreach (var range in ranges)
                {
                    Console.WriteLine(Slice(text, (range.Item1, range.Item2 + 1)));
                }
            }
        }
    }
